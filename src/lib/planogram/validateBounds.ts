...
export function validatePlanogramBounds(p: Planogram): BoundsViolation[] {
  ...
  for (const f of p.sku_facings) {
    ...
    if (!shelf) {
      violations.push({ sku: f.sku, shelf_unit_id: f.shelf_unit_id, reason: 'unknown_shelf', detail: `стеллаж ${f.shelf_unit_id} не найден` });
      continue;
    }
    if (f.shelf_no < 1 || f.shelf_no > shelf.shelf_count) {
      violations.push({ sku: f.sku, shelf_unit_id: f.shelf_unit_id, reason: 'bad_shelf_no', detail: `полка ${f.shelf_no} вне 1..${shelf.shelf_count}` });
      continue;
    }
    const end = f.x_mm + f.width_mm * f.facings;
    if (end > shelf.width_mm) {
      violations.push({ sku: f.sku, shelf_unit_id: f.shelf_unit_id, reason: 'out_of_shelf', detail: `фейсинг заканчивается на ${end} мм > ширина полки ${shelf.width_mm} мм` });
    }
  }
  return violations;
}
